...
=======
>>>>>>> b9010d39ec3b9afe422d4ff7dcbc4d8cc4a6bd24
...
ggplot() +
  ...
  geom_sf(data = tubewell_map, color = "blue") +
  geom_sf(data = )
  xlab("Longitude [degrees]"q
       ) + 
  ylab("Latitude [degrees]") +   
  labs(title = "Tubewell & Diarrheal Treatment Centers in Cox's Bazar, Bangladesh") 

  
Ukhia_med_data <- filtered_df[filtered_df$lat_deg > 21.15 & filtered_df$lat_deg < 21.23, ]
Teknaf_med_data <- filtered_df[filtered_df$lat_deg < 21 & filtered_df$lat_deg > 20.90, ]
...
ggplot() +
  geom_sf(data = diarrhea, color = "green") +
  geom_sf(data = latrine_map, color = "pink") +
  geom_sf(data = tubewell_map, color = "blue") +
  geom_sf(data = Teknaf_data) +
...
```{r}
#Load in household data from WB CBPS Tracking survey, round 1 
# library(haven)
# hhdata <- read_csv("cbps/cbps_tracking_r1_basic needs.csv") 

wells <- tubewells_sf %>%
  filter(`Type of Facilities`=="Handpump-Tubewell") 

latrines <- tubewells_sf %>% 
  filter(`Type of Facilities`=="lat")

well_graph <- ggplot() +
  geom_sf(data = filtered_df, size=0.05, alpha=0.5, color = "green") + 
  geom_sf(data = wells, size=0.05, alpha=0.5, color = "blue") +
  geom_sf(data = camp_outlines, fill = "transparent", color = "yellow") +
  theme_minimal() +
  xlab("Longitude [degrees]") +  # Add x-axis label
  ylab("Latitude [degrees]") +  # Add y-axis label
