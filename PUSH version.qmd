...
This database give sus geospatial information of Bangladesh's administrative divisions.
```{r}
...
ggplot() +
  geom_sf(data = filtered_districts, aes(fill = ADM3_EN), color = "black") +
  geom_sf(data = camp_outlines, aes(color = "yellow"), fill = NA) +
...
plot_filtered_district <- function(district_filter, water_point_filter, camp_outline_filter) {
  ...
  # Plotting
  ggplot() +
    geom_sf(data = new_filtered_districts, aes(fill = ADM3_EN), color = "black") +
    geom_sf(data = new_filtered_camp_outlines, aes(color = "yellow"), fill = NA) +
...
We still need to disagregate further, since there is still almost full overlap between water points and refuge camps data. 
